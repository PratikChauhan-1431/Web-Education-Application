...
if(!isset($_SESSION['id']))
{
	header ('location:index.php');
			...
			{
					$q="select * from chatlog";
					$result=mysqli_query($conn,$q);		
					
					if(isset($_POST['submitmsg']))
					{
						$uid=$_SESSION['id'];
						$sqln="Select * from users where email='$uid'";
						$resultn=mysqli_query($conn,$sqln);
						$rown=mysqli_fetch_array($resultn,MYSQLI_ASSOC);
						$uname=$_SESSION['id'];
						$msg=$_POST["usermsg"]; 
						if (strlen($msg) > 0 && strlen(trim($msg)) == 0 || $msg=='')
						{
							header('location:forum.php');
						}
						else{
						$q="insert into chatlog (name,msg) values ('$uname','$msg')";
						...
						{
							echo"<script>alert('Technical Error.. Message not sent');</script>";
						}						
						}
					}
	  ...
	  <div class='content chat-scroll' id='chat-scroll' style='overflow-y:scroll; height:700px;'>
                        ...
                        <div class='card'>
							...
							echo"</tbody>
								...
								<form id='message' method='POST'>
									<textarea class='form-control' name='usermsg' id='usermsg' size='80' maxlength='80' placeholder='Write you question or query here (maximum of 80 words)'></textarea>
									<br>
									<input name='submitmsg' type='submit'  id='submitmsg' value='Send' class='btn btn-info btn-fill btn-wd' />
								</form>
								<br>
								</div>
							 </center>
